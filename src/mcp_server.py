...
if __name__ == "__main__":
    ...
    pass_tool = next((t for t in tools if t.get("name") == "monthly_pass_registration"), None)
    if pass_tool and not pass_tool.get("parameters", {}).get("properties"):
        print("⏳ [TODO 1.2]: Tool 'monthly_pass_registration' chưa được định nghĩa properties trong 'src/tools.py'.")
    else:
        print("✅ [TODO 1.2]: Tool 'monthly_pass_registration' đã có schema đầy đủ.")

    # Kiểm tra trạng thái TODO 2.1 (call_tool)
    test_result = server.call_tool("route_lookup", {"origin": "Vinhomes Central Park", "destination": "Bến xe Miền Đông mới"})
    if not test_result:
